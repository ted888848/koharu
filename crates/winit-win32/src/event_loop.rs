...
use windows_sys::Win32::UI::WindowsAndMessaging::{
    ...
    EnumThreadWindows, GCLP_WNDPROC, GW_OWNER, GWL_STYLE, GWL_USERDATA, GetClassLongPtrW,
    GetClientRect, GetCursorPos, GetMenu, GetWindow, HTCAPTION, HTCLIENT, LoadCursorW, MINMAXINFO,
    MNC_CLOSE, MSG, MWMO_INPUTAVAILABLE, MsgWaitForMultipleObjectsEx, NCCALCSIZE_PARAMS,
    PEN_FLAG_BARREL, PEN_FLAG_ERASER, PEN_MASK_PRESSURE, PEN_MASK_ROTATION, PEN_MASK_TILT_X,
    PEN_MASK_TILT_Y, PM_REMOVE, PT_PEN, PT_TOUCH, PeekMessageW, PostMessageW, QS_ALLINPUT,
    RI_MOUSE_HWHEEL, RI_MOUSE_WHEEL, RegisterClassExW, RegisterWindowMessageA, SC_MINIMIZE,
    SC_RESTORE, SIZE_MAXIMIZED, SPI_GETWHEELSCROLLCHARS, SPI_GETWHEELSCROLLLINES, SWP_NOACTIVATE,
    SWP_NOMOVE, SWP_NOSIZE, SWP_NOZORDER, SetCursor, SetWindowPos, SystemParametersInfoW,
    TranslateMessage, WHEEL_DELTA, WINDOWPOS, WM_CAPTURECHANGED, WM_CLOSE, WM_CREATE, WM_DESTROY,
...
impl WindowData {
    fn send_window_event(&self, window: HWND, event: WindowEvent) {
        ...
        self.event_loop_runner
            .send_event(Event::Window { window_id, event });
    }
...
impl ThreadMsgTargetData {
    ...
    fn send_device_event(&self, device_id: DeviceId, event: DeviceEvent) {
        self.event_loop_runner
            .send_event(Event::Device { device_id, event });
    }
}
...
impl RootActiveEventLoop for ActiveEventLoop {
    ...
    fn system_theme(&self) -> Option<Theme> {
        Some(if super::dark_mode::should_use_dark_mode() {
            Theme::Dark
        } else {
            Theme::Light
        })
...
unsafe fn reposition_owned_windows(parent: HWND) {
    unsafe extern "system" fn enum_proc(hwnd: HWND, lparam: LPARAM) -> BOOL {
        ...
        // CEF creates top-level windows on this thread and uses GWL_USERDATA for its own state.
        // Only Winit window classes store a WindowData pointer there.
        if unsafe { GetClassLongPtrW(hwnd, GCLP_WNDPROC) }
            != public_window_callback as *const () as usize
        {
            return true.into();
        }
...
fn tablet_tool_info_for_pen(pointer_id: u32) -> (u32, TabletToolData) {
    ...
    util::GET_POINTER_PEN_INFO.map(|GetPointerPenInfo| {
        ...
        if pen_info.penMask & (PEN_MASK_TILT_X | PEN_MASK_TILT_Y) != 0 {
            tool_data.tilt = Some(TabletToolTilt {
                x: pen_info.tiltX as i8,
                y: pen_info.tiltY as i8,
            });
        }
