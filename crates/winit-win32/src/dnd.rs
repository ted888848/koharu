...
impl DataObject {
    ...
    fn resolve(&self, requested: TypeHint) -> Option<TypeHint> {
        self.data
            .keys()
            .copied()
            .find(|stored| stored.matches(&requested))
    }
}
...
impl FileDropHandler {
    ...
    unsafe extern "system" fn DragEnter(
        ...
        drop_handler
            .runner
            .register_data_transfer(data_transfer_id, wid, data);
    ...
    unsafe extern "system" fn DragOver(
        ...
        let proposed_action = drop_handler
            .runner
            .proposed_dnd_action(data_transfer_id, effects);
    ...
    unsafe extern "system" fn DragLeave(this: *mut IDropTarget) -> HRESULT {
        ...
        (drop_handler.send_event)(WindowEvent::DragLeft {
            id: data_transfer_id,
        });
    ...
    unsafe extern "system" fn Drop(
        ...
        if drop_handler.runner.source_drag.get().is_some() {
            drop_handler
                .runner
                .defer_source_drag_cleanup(data_transfer_id);
        } else {
            drop_handler.runner.remove_data_transfer(data_transfer_id);
        }
    ...
    let hglobal = match data {
        ...
        SendData::Uris(paths) => {
            ...
            for path in paths {
                ...
                let encoded = if let Some(path_from_uri) = url::Url::parse(&path)
                    .ok()
...
fn variant_matches_hint(hint: TypeHint, data: &SendData) -> bool {
    matches!(
        ...
        (TypeHint::UriList, SendData::Uris(_))
            | (
                TypeHint::Plaintext | TypeHint::Html | TypeHint::Rtf,
                SendData::String(_)
            )
            | (
...
impl SourceDataObjectData {
    ...
    unsafe extern "system" fn SetData(
        ...
        if let Some(slot) = extras
            .iter_mut()
            .find(|(f, _)| f.cfFormat == format.cfFormat)
        {
            slot.0 = format;
...
impl SourceDataObject {
    pub(crate) fn new(send_data: Box<dyn DataTransferSend>) -> Self {
        Self {
            data: SourceDataObjectData::new_boxed(send_data),
        }
    }
...
pub(crate) unsafe fn apply_drag_image(
    ...
    let sdi = SHDRAGIMAGE {
        sizeDragImage: windows_sys::Win32::Foundation::SIZE {
            cx: width as i32,
            cy: height as i32,
        },
        ptOffset: POINT {
