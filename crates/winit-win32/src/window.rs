...
impl CoreWindow for Window {
        ...
        self.thread_executor.execute_in_thread(move || {
            ...
            match (&old_fullscreen, &fullscreen) {
                (_, Some(Fullscreen::Exclusive(monitor, video_mode))) => {
                    ...
                    let video_mode = match monitor
                        .video_mode_handles()
                        .find(|mode| &mode.mode == video_mode)
                    {
    ...
    fn current_monitor(&self) -> Option<CoreMonitorHandle> {
        Some(CoreMonitorHandle(Arc::new(monitor::current_monitor(
            self.hwnd(),
        ))))
    }
...
unsafe fn init(
    ...
    window_flags.set(
        ...
        win_attributes.decoration_shadow,
    );
    window_flags.set(
        WindowFlags::ALWAYS_ON_TOP,
    ...
    window_flags.set(
        ...
        win_attributes.no_redirection_bitmap,
    );
